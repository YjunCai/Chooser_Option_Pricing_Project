"""
Chooser Option Pricing Tool -- Streamlit Dashboard
==================================================
Complete visualization dashboard for the deployable chooser-option pricing tool:

  * dual pricing -- BSM(vol_21d) baseline vs the best ML volatility models
    (XGBoost = live pick, VIX-proxy = IV-aligned, GBDT-anchored = ratio), priced
    with the validated Rubinstein chooser formula;
  * error-margin displays -- Week-6 test-set MAE/RMSE attached to every price;
  * full visualization dashboard -- 6 tabs:
      1. price trends over time (2018-2024 training + 2025+ out-of-sample live),
      2. interactive sensitivity (spot / vol / choice date / rate),
      3. extreme scenarios (vol +50%, rate +2%, combined, VIX shock),
      4. performance metrics (chooser, vol, live targets, end-to-end),
      5. error margins, 6. real-time data;
...
def cached_live_state():
    ...
    row = feat.iloc[-1]
    return {
        'row': row, 'spot': float(row[config.SPOT_COL]),
        'rate': float(row[config.RATE_COL]) / 100.0,
        'q': config.Q_YIELD, 'vol21': float(row['vol_21d']),
        'vix': float(row[config.VIX_COL]), 'last_date': str(feat.index[-1].date()),
    }
...
def cached_trend_series():
    return db.historical_price_series(max_rows=config.PRICE_SERIES_SAMPLE)
...
def predict_sigma(family: str, row: pd.Series) -> float:
    ...
    sub = pd.Series(row[feats].values, index=feats)   # align to the model's feature list
    return float(te.predict_sigma_family(family, payload, sub, float(row['vol_21d'])))
...
def price_arrays(S0, K, t1, T2, r, q, sigma_ml, sigma_base, grid, which):
    ...
    if which == 'spot':
        return grid, te.price_chooser_vec(grid, K, t1, T2, r, q, sig_ml), \
            te.price_chooser_vec(grid, K, t1, T2, r, q, sig_base)
    if which == 'sigma':
        return grid, te.price_chooser_vec(np.full(len(grid), S0), K, t1, T2, r, q, grid), \
            te.price_chooser_vec(np.full(len(grid), S0), K, t1, T2, r, q, grid)
    if which == 't1':
        return (grid,
                [te.price_chooser(S0, K, g, T2, r, q, sigma_ml) for g in grid],
                [te.price_chooser(S0, K, g, T2, r, q, sigma_base) for g in grid])
    if which == 'rate':
        return (grid,
                [te.price_chooser(S0, K, t1, T2, g, q, sigma_ml) for g in grid],
                [te.price_chooser(S0, K, t1, T2, g, q, sigma_base) for g in grid])
...
with st.sidebar:
    st.title('⚙️ 参数设置')
    st.caption('Chooser 定价工具 · 双轨定价 (BSM + 最优 ML 模型)')
    ...
    st.subheader('合约参数')
    K = st.number_input('行权价 K ($)', value=float(config.CHOOSER_PARAMS['K']), step=5.0)
    t1 = st.slider('选择日 t1 (年)', 0.05, 1.0, float(config.CHOOSER_PARAMS['t1']), 0.05)
    T2 = st.slider('最终到期 T2 (年)', 0.25, 3.0, float(config.CHOOSER_PARAMS['T2']), 0.25)
    S = st.number_input('标的现价 S ($)', value=float(live['spot']), step=5.0)
    r_pct = st.slider('无风险利率 r (%)', 0.0, 12.0, float(live['rate'] * 100), 0.1)
    q_pct = st.slider('股息率 q (%)', 0.0, 5.0, float(config.Q_YIELD * 100), 0.1)
...
st.caption(f'双轨定价 (BSM(vol_21d) + 最优 ML 波动率模型) · 数据日期 {live["last_date"]} '
           '· 模型来自 models/*.pkl')
...
c2.metric('ML 定价', f"${p['price_ML']:.4f}", delta=f"{p['spread_$']:+.4f} (ML-BSM)")
c3.metric('ML 波动率', f"{p['sigma_ML']*100:.2f}%")
c4.metric('BSM 波动率', f"{p['sigma_base']*100:.2f}%")

st.caption(f"模型: {config.MODEL_LABELS.get(family, family)}  |  "
...
with tab3:
    ...
    base_row, spot0, rate0, q0, vol21 = live['row'], live['spot'], live['rate'], config.Q_YIELD, live['vol21']
    sc = sens.extreme_scenarios(base_row, S, r, q, sigma_base, verbose=False)
    st.dataframe(sc.round(3), width='stretch', hide_index=True)
    st.caption('表中价格为当前合约参数下的取值；波动率激增同时放大 ML 预测 σ，故 ML 价格变动通常大于 BSM。')
...
with tab4:
    ...
    with c_left:
        st.image(str(config.ASSETS_DIR / 'fig_chooser_metrics.png'), width='stretch')
        st.image(str(config.ASSETS_DIR / 'fig_vol_metrics.png'), width='stretch')
    with c_right:
        st.image(str(config.ASSETS_DIR / 'fig_live_metrics.png'), width='stretch')
        st.image(str(config.ASSETS_DIR / 'fig_end2end.png'), width='stretch')
    st.caption('图(左上) 方法一 Chooser 定价测试集误差; (右上) 方法一波动率 MAE; '
               '(左下) 实盘快照目标对照; (右下) 方法二端到端定价。')
    with st.expander('查看明细表'):
        for title, key in [('方法一 · Chooser 定价测试集 (2024)', 'chooser'),
                           ('方法一 · 波动率预测测试集 (2024)', 'vol'),
                           ('实盘快照指标 (2026-07-27)', 'live'),
                           ('方法二 · 端到端定价测试集', 'end2end')]:
            st.markdown(f'**{title}**')
            st.dataframe(mtr[key].round(3), width='stretch', hide_index=True)
...
with tab5:
    ...
    st.metric(f"当前模型 ({config.MODEL_LABELS.get(family, family)}) 定价 ± MAE",
